...
public class SamlUtil {
    ...
    public static BasicX509Credential publicKeyCredential(String samlCerPath) throws IOException, CertificateException {
        FileInputStream input = new FileInputStream(new File(samlCerPath));
        try {
    ...
    private static Optional<List<Assertion>> decryptSaml(Response samlResponse, BasicX509Credential publicKeyCredential) {
        ...
        if (assertions != null) {
            return Optional.of(assertions.stream().map(assertion -> {
                try {
                ...
                return null;
            }).filter(assertion -> assertion != null).collect(Collectors.toList()));
        }
    ...
    private static Response getSamlResponse(String saml){
        ...
        try {
            ...
            Document document = docBuilder.parse(new ByteArrayInputStream(saml.getBytes("UTF-8")));
